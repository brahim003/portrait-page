...
const resolution = 250; // عدد الحروف عرضاً (زدها لزيادة الدقة)
...
const height = Math.floor(width * scale * 0.7);
    canvas.width = width;
...
window.addEventListener('wheel', (e) => {
    if (e.ctrlKey) {
        e.preventDefault();
        const container = document.getElementById('portrait-container');
        let currentScale = parseFloat(container.style.transform.replace('scale(', '')) || 1;
